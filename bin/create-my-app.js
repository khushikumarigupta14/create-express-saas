...
async function copyTemplate(targetDir, projectName) {
  await fse.copy(TEMPLATE_DIR, targetDir, { overwrite: true, errorOnExist: false });
    ...
    try {
      ...
      if (fs.existsSync(targetDir)) {
        ...
        if (files.length > 0 && !options.force) {
          spinner.fail(`Target directory ${chalk.yellow(targetDir)} is not empty. Use --force to overwrite.`);
          process.exit(1);
        }
      } else {
